...
def create_dataset(iterations = list, calculus = list, number = int):
    ...
    inpt, y_train_ordered, max_y_train_len = dataset_from_derivations(drvas) # Generate onehot encoded input dataset "inpt", output dataset "y_train_ordered" and get maximum length of derivations in y as number
    ...
    # Print results
    print(f"Number of input entries: {len(inpt)}")
    print(f"Average ground truth entries/ unique premise pairs: {len(drvas)/len(y_train_ordered)}")
    return inpt_2d, inpt, y_train_ordered, max_y_train_len # Return the two and three dimensional input dataset, the output datset and the maximum length of derivations in the latter
...
def dataset_from_derivations(drvas):
    y_train_ordered = []
    ...
    onehot_drvas = np.array([sub + [0] * (maxl - len(sub)) for sub in tqdm(onehot_drvas, desc =  "Padded x_train entries", file=sys.stdout)]) # Pad all derivation entries with zeros to length of longest derivation
    onehot_drvas = torch.tensor(onehot_drvas) 
    onehot_drvas = [torch.cat([F.one_hot(i[j], num_classes=t_nu + 9) for j in range(len(i))], dim=-1) for i in tqdm(onehot_drvas, file=sys.stdout)]
    max_l = len(onehot_drvas[0])
    n = len(drvas)

    for i, (drv, one_drv) in tqdm(enumerate(zip(drvas, onehot_drvas)), desc = "Processed entries for x_train and y_tdict",total = n, file=sys.stdout):
    ...
    padded_y_train_ordered = [sub + [torch.tensor(y_padding)] * (max_y - len(sub)) for sub in tqdm(y_train_ordered, desc="Padded y_train_ordered", file=sys.stdout)]
...
def parallel_pad(input):
    ...
    with Pool(processes=num_cores) as pool:
        ...
        results = []
        for result in tqdm(pool.imap_unordered(func, input), total=len(input), file=sys.stdout):
...
def list_to_onehot(list):
    ...
    inpt = np.array([sub + [0] * (maxl - len(sub)) for sub in tqdm(inpt, desc =  "Padded x_train entries", file=sys.stdout)]) # Pad with zeros to length of longest derivation
